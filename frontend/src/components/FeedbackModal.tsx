...
export default function FeedbackModal({ page, onClose }: FeedbackModalProps) {
  ...
  const handleSubmit = async () => {
    if (rating === 0) { setError(t('feedback.ratingLabel')); return; }
    try {
      await api.submitFeedback({
        page, rating, text: text || undefined,
    ...
    return (
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
        <div className="bg-white rounded-xl p-6 max-w-sm w-full mx-4 text-center">
          ...
          <h3 className="text-lg font-semibold mb-1">{t('feedback.thanks')}</h3>
          <button onClick={onClose} className="mt-3 px-4 py-2 bg-gray-900 text-white rounded-lg text-sm">
            {t('common.close')}
          </button>
        </div>
      </div>
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-6 max-w-sm w-full mx-4">
        ...
        <div className="flex gap-2 mb-4">
          {[1, 2, 3, 4, 5].map((star) => (
            <button key={star} onClick={() => { setRating(star); setError(''); }}
        ...
        <textarea value={text} onChange={(e) => setText(e.target.value)}
          ...
          className="w-full border rounded-lg p-3 text-sm mb-3 resize-none" rows={3} />
